...
interface TechnicalClientProps {
    lang: LangType;
}

export default function TechnicalClient({ lang }: TechnicalClientProps) {
    const isJa = lang === 'JP';
    const isRTL = lang === 'AR';
    ...
    return (
        <div className="min-h-screen bg-black text-slate-200 font-sans selection:bg-sky-500/30 pb-20">
            ...
            <div className="max-w-[1200px] mx-auto p-4 md:p-12 lg:p-16">
                ...
                <header className={`mb-12 border-b border-slate-800 pb-8 ${isRTL ? 'text-right' : 'text-left'}`}>
                    <div className={`flex justify-between items-start mb-4`}>
                        <h1 className={`text-3xl md:text-5xl font-black text-white tracking-tight uppercase flex flex-col gap-1`}>
                            <span className="text-xl md:text-2xl text-slate-500 tracking-widest">{DICTIONARY[lang].labels.technical}</span>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {category.indicators.map((indicator, index) => {
                                        ...
                                        return (
                                            <div
                                                key={index}
                                                className="group relative bg-[#0A0A0A] border border-slate-800 hover:border-purple-500/50 transition-colors duration-300 rounded-xl overflow-hidden shadow-lg hover:shadow-purple-500/10"
                                            >
                                                <div className="p-6 flex flex-col h-full">
                                                    <div className={`flex items-start justify-between mb-4`}>
                                                        {showWiki ? (
                                                            <a href={lang.toLowerCase() === 'en' ? `/wiki/${wikiSlug}` : `/${lang.toLowerCase()}/wiki/${wikiSlug}`} className="group/title flex items-center gap-3 w-full hover:opacity-80 transition-opacity">
                                                                <h3 className={`font-bold text-lg text-slate-100 group-hover/title:text-purple-500 transition-colors ${isRTL ? 'text-right' : ''}`}>
                                                                    {indicator.name}
                                                                </h3>
                                                                <Activity className={`w-5 h-5 text-slate-600 group-hover/title:text-purple-500 transition-colors flex-shrink-0 ${isRTL ? 'mr-auto' : 'ml-auto'}`} />
                                                            </a>
                                                        ) : (
                                                            <>
                                                                <h3 className={`font-bold text-lg text-slate-100 transition-colors ${isRTL ? 'text-right' : ''}`}>
                                                                    {indicator.name}
                                                                </h3>
                                                                <Activity className={`w-5 h-5 text-slate-600 transition-colors flex-shrink-0 ${isRTL ? 'mr-auto' : 'ml-auto'}`} />
                                                            </>
                                                        )}
                                                    </div>
                                                    ...
                                                    <div className={`mt-auto pt-6 border-t border-slate-800 bg-[#111111] -mx-6 -mb-6 p-6`}>

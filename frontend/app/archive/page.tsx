        ...
        <TerminalPage pageKey="archive">
            <div className="max-w-3xl space-y-12">
                ...
                <div className="bg-sky-50 dark:bg-sky-500/5 border border-sky-200 dark:border-sky-500/20 rounded-xl overflow-hidden">
                    ...
                    <div className="p-8 space-y-8">
                        {auditLoading ? (
                            ...
                            <>
                                <p className="text-lg text-slate-700 dark:text-slate-200 font-medium leading-relaxed italic border-l-2 border-sky-500/50 pl-6">
                                    {auditData.summaries[lang] || auditData.summaries['EN']}
                                </p>

                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                                    <div className="bg-white dark:bg-white/5 p-4 rounded-lg border border-slate-200 dark:border-white/5 shadow-sm dark:shadow-none">
                                        <span className="text-[10px] text-slate-500 block uppercase font-bold tracking-widest mb-1 text-red-500 dark:text-red-400/80">Avg VIX (Defensive)</span>
                                        <span className="text-2xl font-black text-slate-900 dark:text-white italic">{auditData.metrics.avg_vix_defensive || '--'}</span>
                                    </div>
                                    <div className="bg-white dark:bg-white/5 p-4 rounded-lg border border-slate-200 dark:border-white/5 shadow-sm dark:shadow-none">
                                        <span className="text-[10px] text-slate-500 block uppercase font-bold tracking-widest mb-1 text-slate-500 dark:text-slate-400">Avg VIX (Neutral)</span>
                                        <span className="text-2xl font-black text-slate-900 dark:text-white italic">{auditData.metrics.avg_vix_neutral || '--'}</span>
                                    </div>
                                    <div className="bg-white dark:bg-white/5 p-4 rounded-lg border border-slate-200 dark:border-white/5 shadow-sm dark:shadow-none">
                                        <span className="text-[10px] text-slate-500 block uppercase font-bold tracking-widest mb-1 text-sky-600 dark:text-sky-400/80">Avg VIX (Accumulate)</span>
                                        <span className="text-2xl font-black text-slate-900 dark:text-white italic">{auditData.metrics.avg_vix_accumulate || '--'}</span>
                                    </div>
                                </div>
                            </>
                        ) : (
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-[#0A0A0A] border border-slate-200 dark:border-white/10 rounded-xl overflow-x-auto shadow-sm dark:shadow-none">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                                {auditLoading ? (
                                    ...
                                    [...auditData.history].reverse().map((row: any) => (
                                        <tr key={row.date} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
                                            <td className="p-4 font-mono text-xs text-slate-600 dark:text-slate-400">{row.date}</td>
                                            <td className="p-4">
                                                <div className="flex items-center gap-2">
                                                    <div className={`w-2 h-2 rounded-full ${row.gms_score > 60 ? 'bg-sky-500' : row.gms_score < 40 ? 'bg-red-500' : 'bg-amber-500'}`} />
                                                    <span className="font-bold text-sm italic text-slate-900 dark:text-slate-100">{row.gms_score}</span>
                                                </div>
                                            </td>
                                            <td className="p-4 font-mono text-xs text-slate-900 dark:text-white">${row.spy_price || '--'}</td>
                                            <td className="p-4 font-mono text-xs text-slate-600 dark:text-slate-400">{row.net_liquidity || '--'}</td>
                                            <td className="p-4 font-mono text-xs text-slate-600 dark:text-slate-400">{row.vix_price || '--'}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
                ...
                {loading ? (
                    <div className="space-y-4 animate-pulse">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="h-16 bg-slate-100 dark:bg-[#111] border border-slate-200 dark:border-white/5 rounded-[2px]"></div>
                        ))}
                    </div>
                ) : (

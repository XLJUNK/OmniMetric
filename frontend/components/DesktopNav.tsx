...
export const DesktopNav = ({ lang: defaultLang }: { lang: LangType }) => {
    ...
    const navItems = [
        ...
        { key: 'currencies', label: t.labels.currencies || 'CURRENCIES', path: '/currencies' },
    ...
    return (
        <div className="hidden md:block w-full border-b border-slate-800 bg-black sticky top-0 z-50 shadow-md">
            <div className="max-w-[1600px] mx-auto flex items-center justify-start gap-1">
                {navItems.map((item) => {
                    const localizedPath = lang.toUpperCase() === 'EN'
                        ? item.path
                        : (item.path === '/' ? `/${lang.toLowerCase()}` : `/${lang.toLowerCase()}${item.path}`);

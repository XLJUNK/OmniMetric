...
export const GlossaryClient = ({ lang, data, pageTitle, pageDesc, searchItems, searchPlaceholder }: GlossaryClientProps) => {
    ...
    return (
        <div className="max-w-[1600px] mx-auto flex flex-col lg:flex-row">
            ...
            {/* SIDEBAR TOC (Desktop) */}
            <aside className={`hidden lg:block w-[300px] py-12 sticky top-[60px] h-[calc(100vh-60px)] overflow-y-auto ${isRTL ? 'border-l border-[#1e293b]/50 pl-6 pr-8' : 'border-r border-[#1e293b]/50 pl-8 pr-6'}`}>
                ...
                <nav className="space-y-6">
                    {categories.map((cat) => (
                        <div key={cat}>
                            <h3 className={`text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 border-b border-slate-800 pb-1 ${isRTL ? 'text-right' : 'text-left'}`}>
                                {cat}
                            </h3>
                            <ul className="space-y-1">
                                {groupedData[cat].map((item) => (
                                    <li key={item.id}>
                                        <a
                                            href={`#${item.id}`}
                                            className={`block text-[11px] text-slate-400 hover:text-white py-1 border-transparent hover:border-sky-500 transition-all truncate ${isRTL ? 'text-right pr-2 border-r-2 hover:border-r-sky-500' : 'text-left pl-2 border-l-2 hover:border-l-sky-500'}`}
                                        >
                                            {item.term}
                                        </a>
                                    </li>
            ...
            <main className="flex-1 p-4 md:p-12 lg:p-16">
                <header className={`mb-16 border-b border-[#1E293B] pb-8 ${isRTL ? 'text-right' : 'text-left'}`}>
                    <h1 className={`text-3xl md:text-5xl font-black text-white tracking-tight mb-4 flex gap-3 ${isRTL ? 'justify-start' : ''}`}>
                        Strategic <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-emerald-400">{pageTitle}</span>
                    </h1>
                                        ...
                                        <React.Fragment key={item.id}>
                                            ...
                                            >
                                                ...
                                                <div className="p-6 md:p-8">
                                                    ...
                                                    {/* Definition */}
                                                    <p className={`text-slate-300 leading-relaxed mb-8 text-sm md:text-base ${isRTL ? 'text-right border-r-2 border-slate-700 pr-4' : 'text-left border-l-2 border-slate-700 pl-4'}`}>
                                                        {item.definition}
                                                    </p>
                                                    ...
                                                    <div className={`grid grid-cols-1 xl:grid-cols-2 gap-8`}>
                                                        ...
                                                        <div className="space-y-4">
                                                            ...
                                                            <div className={`border-slate-800/50 space-y-4 ${isRTL ? 'pr-4 border-r' : 'pl-4 border-l border-[#222]'}`}>
                                                                {/* UP */}
                                                                <div className="flex flex-col gap-1">
                                                                    ...
                                                                    <span className={`text-slate-400 text-xs leading-relaxed block ${isRTL ? 'pr-7 text-right' : 'pl-7 text-left'}`}>
                                                            ...
                                                            <div className={`mt-4 ${isRTL ? 'pr-4 border-r border-[#222]' : 'pl-4 border-l border-[#222]'}`}>
                                                                <TVPartnerCard lang={lang} variant="text-link" />
                                                            </div>
                                                        </div>

                                                        {/* Context 2026 (Right Col) */}
                                                        <div className={`bg-sky-950/10 border border-sky-900/20 rounded-lg p-5 relative ${isRTL ? 'text-right' : 'text-left'}`}>
                                                            ...
                                                            <p className={`text-sky-200/80 text-xs md:text-sm font-serif italic leading-relaxed relative z-10 ${isRTL ? 'pr-2 border-r-2 border-sky-500/30' : 'pl-2 border-l-2 border-sky-500/30'}`}>
                                                                "{item.context_2026}"
                                                            </p>
                                                        </div>
                                                    </div>
